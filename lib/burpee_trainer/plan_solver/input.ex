defmodule BurpeeTrainer.PlanSolver.Input do
  @moduledoc """
  Canonical input to `BurpeeTrainer.PlanSolver.solve/1`.

  UI/editor compatibility fields are translated before this boundary. The solver
  consumes seconds, explicit rest structs, max unbroken reps, and sec-per-rep
  overrides only.
  """

  alias BurpeeTrainer.PlanSolver.{BlockSpec, ExplicitRest, Infeasible}

  @enforce_keys [:burpee_type, :target_duration_sec, :burpee_count_target, :pacing_style]
  defstruct [
    :name,
    :burpee_type,
    :target_duration_sec,
  ...
  def normalize_and_validate(%__MODULE__{} = input) do
    input =
      ...
      |> normalize_block_pattern_structure()

    validate_canonical(input)
  end

  defp normalize_explicit_rests(%__MODULE__{explicit_rests: rests} = input) when is_list(rests),
    do: input
  ...
  defp normalize_block_pattern_structure(
         %__MODULE__{
  ...
  defp normalize_block_pattern_structure(input), do: input
