...
const PoseDebug = {
  ...
  draw(pose) {
    ...
    for (const [aName, bName] of EDGES) {
      ...
      this.ctx.beginPath()
      this.ctx.moveTo(scaleX(a.x, this.video, rect), scaleY(a.y, this.video, rect))
      this.ctx.lineTo(scaleX(b.x, this.video, rect), scaleY(b.y, this.video, rect))
      this.ctx.stroke()
    }
    ...
    for (const point of pose.keypoints) {
      ...
      this.ctx.beginPath()
      this.ctx.arc(scaleX(point.x, this.video, rect), scaleY(point.y, this.video, rect), 5, 0, Math.PI * 2)
      this.ctx.fill()
    }
  ...
  renderStats(sample, now) {
    ...
    setText(this.el, "#pose-debug-cadence", JSON.stringify(this.state.cadenceMs))
  },
