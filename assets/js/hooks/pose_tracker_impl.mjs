...
export function createPoseTracker(hook) {
  ...
  async function mountedHook() {
    ...
    try {
      ...
      hook.pushEvent("track", { state: "lost", reason: error?.name || "tracker_error" })
    }
  }
  ...
  async function loop() {
    ...
    const sample = sampleFromPose(poses[0], performance.now() - startedAt, video)
    ...
    if (result.rep) {
      hook.pushEvent("rep", { index: state.cadenceMs.length, t_ms: sample.tMs })
    }
  ...
  function finish(event) {
    ...
    try {
      hook.pushEvent("finish", buildFinishPayload({ durationMs, cadenceMs: state.cadenceMs }))
    } catch (_error) {
