import { DEFAULT_THRESHOLDS, initialFsmState, stepFsm } from "./pose_rep_fsm.mjs";
...
export function countRep(state, sample, thresholds = DEFAULT_THRESHOLDS) {
	const signalMin = state.signalMin == null ? sample.signal : Math.min(state.signalMin, sample.signal);
	const signalMax = state.signalMax == null ? sample.signal : Math.max(state.signalMax, sample.signal);
	const adaptiveThresholds = thresholdsForRange(thresholds, signalMin, signalMax);
